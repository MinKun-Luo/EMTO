...
class MPPSO(Algorithm):
    ...
    def run(self, Prob, isPrint=False):
        ...
        # 初始化种群
        population = Initialization(self, Prob, Individual_PSO)
        # 初始化个体的最优位置和最优值
        for t in range(Problem.T):
            for ind in population[t]:
        ...
        while self.notTerminated(Prob, isPrint):
            ...
            for t in range(Problem.T):
                population[t] = self.Generation(population[t], t, W)
                # 临时种群
                population_temp = copy.deepcopy(population[t])
                # 选择性评估
                self.Evaluation(population[t], Prob[t], t)

                # 更新个体的最优位置和最优值
                for i in range(len(population[t])):
                    if population[t][i].obj < population_temp[i].pBestObj:
                        population[t][i].pBestRnvec = copy.deepcopy(population[t][i].rnvec)
                        population[t][i].pBestObj = population[t][i].obj

        return self

    def Generation(self, population, t, W):
        """
        生成子代种群。
